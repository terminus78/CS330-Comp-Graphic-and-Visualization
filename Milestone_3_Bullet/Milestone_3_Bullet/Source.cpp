...
#include <chrono>           // time for animation use
...
namespace
{
	const char* const WINDOW_TITLE = "Shawn Way: A Dangerous Game"; // Macro for window title
	...
	// Texture
	GLuint gTextureIdCasing;
	GLuint gTextureIdBullet;
	GLuint gTextureIdTable;
	GLuint gTextureIdFlag;
	GLuint gTextureIdDie;
	GLuint gTextureIdCardBox;
	...
	// camera
	Camera gCamera(glm::vec3(0.0f, 10.0f, 30.0f));
...
const GLchar * objectVertexShaderSource = GLSL(440,
	...
	layout(location = 1) in vec3 normal; // Normals data from Vertex Attrib Pointer 1
...
const GLchar* objectFragmentShaderSource = GLSL(440,
	...
	in vec3 vertexFragmentPos; // For incoming fragment postion
	...
	void main()
	{
		...
		vec3 norm = normalize(vertexNormal); // Normaliize vectors to 1 unit
		vec3 light1Direction = normalize(light1Pos - vertexFragmentPos); // Calculate distance (light direction) between light source and fragments/pixels on object
		...
		vec3 light2Direction = normalize(light2Pos - vertexFragmentPos); // Calculate distance (light direction) between light source and fragments/pixels on object
		...
		vec4 textureColor = mix(texture(uTexture1, vertexTextureCoordinate), texture(uTexture2, vertexTextureCoordinate), 0.8);

		vec3 phong1 = (ambient + diffuse1 + specular1) * textureColor.xyz;
...
const GLchar* lightFragmentShaderSource = GLSL(330,
	...
	out vec4 fragmentColor; // For outgoing light color (smaller cube) to the GPU
...
int main(int argc, char* argv[])
{
	...
	// Load texture
	const char* texFilename1 = "gold-metal-texture.jpg";
	if (!UCreateTexture(texFilename1, gTextureIdCasing))
	{
		cout << "Failed to load texture " << texFilename1 << endl;
		return EXIT_FAILURE;
	}

	const char* texFilename2 = "golden-texture-background2-392425-pixahive.jpg";
	if (!UCreateTexture(texFilename2, gTextureIdBullet))
	{
		cout << "Failed to load texture " << texFilename2 << endl;
		return EXIT_FAILURE;
	}

	const char* texFilename3 = "olive-fabric-background.jpg";
	...
	const char* texFilename4 = "image-16252994463A0.png";
	if (!UCreateTexture(texFilename4, gTextureIdFlag))
	{
		cout << "Failed to load texture " << texFilename4 << endl;
		return EXIT_FAILURE;
	}

	const char* texFilename5 = "pattern-background-texture-vintage-preview.jpg";
	if (!UCreateTexture(texFilename5, gTextureIdDie))
	{
		cout << "Failed to load texture " << texFilename5 << endl;
		return EXIT_FAILURE;
	}

	const char* texFilename6 = "card-game-48983_1280.png";
	if (!UCreateTexture(texFilename6, gTextureIdCardBox))
	{
	...
	// Release texture
	UDestroyTexture(gTextureIdCasing);
	UDestroyTexture(gTextureIdBullet);
	UDestroyTexture(gTextureIdTable);
	UDestroyTexture(gTextureIdFlag);
	UDestroyTexture(gTextureIdDie);
	UDestroyTexture(gTextureIdCardBox);
...
void UProcessInput(GLFWwindow* window)
{
	...
	// change matrix mode only once when key released
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS && !viewChangeKeyHeld)
	{
		perspectiveView = !perspectiveView;
		viewChangeKeyHeld = true;
	}
	if (viewChangeKeyHeld && glfwGetKey(window, GLFW_KEY_P) == GLFW_RELEASE)
		viewChangeKeyHeld = false;
}
...
void URender()
{
	...
	glClearColor(0.4392f, 0.4863f, 0.5098f, 1.0f);
	...
	if (perspectiveView == true)
		...
		projection = glm::ortho(-7.5f, 7.5f, -7.5f, 7.5f, 0.1f, 100.0f);

	// Set the shader to be used
	glUseProgram(gObjectProgramId);

	// Retrieves and passes transform matrices to the Shader program
	viewPosLoc = glGetUniformLocation(gObjectProgramId, "viewPosition");
	ambStrLoc = glGetUniformLocation(gObjectProgramId, "ambientStrength");
	ambColLoc = glGetUniformLocation(gObjectProgramId, "ambientColor");
	light1ColLoc = glGetUniformLocation(gObjectProgramId, "light1Color");
	light1PosLoc = glGetUniformLocation(gObjectProgramId, "light1Pos");
	light2ColLoc = glGetUniformLocation(gObjectProgramId, "light2Color");
	light2PosLoc = glGetUniformLocation(gObjectProgramId, "light2Pos");
	objColLoc = glGetUniformLocation(gObjectProgramId, "objectColor");
	specInt1Loc = glGetUniformLocation(gObjectProgramId, "specularIntensity1");
	highlghtSz1Loc = glGetUniformLocation(gObjectProgramId, "highlightSize1");
	specInt2Loc = glGetUniformLocation(gObjectProgramId, "specularIntensity2");
	highlghtSz2Loc = glGetUniformLocation(gObjectProgramId, "highlightSize2");
	...
	glUniform3f(light1PosLoc, 10.0f, 30.0f, 25.0f);
	glUniform3f(light2ColLoc, 1.0f, 1.0f, 1.0f);
	glUniform3f(light2PosLoc, 7.0f, 5.0f, -30.0f);
	...
	glUniform1f(highlghtSz1Loc, 16.0f);
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(15.0f, 1.0f, 20.0f));
	// 2. Rotate the object
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureIdCasing);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, gTextureIdFlag);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(1.0f, 3.0f, 1.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-5.0f, 0.0f, -3.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureIdBullet);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, 0);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.95f, 1.3f, 0.95f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-5.0f, 2.75f, -3.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureIdDie);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, 0);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.0f, 1.75f, 1.75f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(180.0f), glm::vec3(0.0, 1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-1.0f, 0.9f, 9.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 3. Position the object
	translation = glm::translate(glm::vec3(3.5f, 0.75f, 7.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gTextureIdCardBox);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, 0);

	// 1. Scales the object
	scale = glm::scale(glm::vec3(6.0f, 1.5f, 10.0f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(30.0f), glm::vec3(0.0, -1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.5f, 0.75f, 1.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// Lights
	// 
	// Diffuse overhead light and light
	// from sun
	///////////////////////////////////////////////////
	...
	//Transform the sphere used as a visual que for the light source
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.5f, 0.5f, 0.5f));
	// 2. Rotates shape
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(10.0f, 30.0f, 25.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Place object at the origin
	translation = glm::translate(glm::vec3(7.0f, 5.0f, -30.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
